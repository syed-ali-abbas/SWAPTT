...
from .scripts.webapp import cve_2021_41773, gather_url, subdomain_finder, verbtampering, corsy_misconfiguration, xssstrike,sqlinjection
...
def corsy(request):
    if request.method == "GET":
        return render(request,"toolkit/webapp/corsy.html")
        ...
        try:
            ...
            if form.is_valid():
                ...
                result = corsy_misconfiguration.get(target_url)
             
                if result is None:
                    return render(
                        request,
                        "toolkit/webapp/corsy.html",
                        {"error": "Bad URL Passed in, Try again..."},
                    )

                else:
                    context = {"result": result, "target_url": target_url}
                    return render(request,"toolkit/webapp/corsy.html",context)
...
def SQLInjection(request):	
    if request.method == "GET":
        return render(request,"toolkit/webapp/sqlInjection.html")
        ...
        try:
            ...
            if form.is_valid():
                target_url = form.cleaned_data.get("target_url")
                result = sqlinjection.get(target_url)
                print(result)
                if result is None:
                    return render(
                        request,
                        "toolkit/webapp/sqlInjection.html",
                        {"error": "Bad URL Passed in, Try again..."},
                    )

                else:
                    context = {"result": result, "target_url": target_url}
                    return render(request, "toolkit/webapp/sqlInjection.html", context)
...
def XSS(request):
    if request.method == "GET":
        return render(request, "toolkit/webapp/XSS.html", {"form": URLForm()})
        ...
        try:
            ...
            if form.is_valid():
                target_url = form.cleaned_data.get("target_url")
                result = xssstrike.get(target_url)
             
                if result is None:
                    return render(
                        request,
                        "toolkit/webapp/XSS.html",
                        {"error": "Bad URL Passed in, Try again..."},
                    )

                else:
                    context = {"result": result, "target_url": target_url}
                    return render(request, "toolkit/webapp/XSS.html", context)
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if form.is_valid():
                ...
                result_ = gather_url.get_(target_url)
                if result is None:
                    return render(
                        request,
                    ...
                    context = {"result": result,"result_":result_, "target_url": target_url}
...
#------------------------------Newly Added Lines--------------(SubdomainTakeover)-------------------
def subdomainTakeover(request):
	result = subdomain_finder.subzy()
	if request.method == "GET":
		context = {
		"results":result
		}
		return render(request,"toolkit/webapp/subdomainTakeover.html",context)
